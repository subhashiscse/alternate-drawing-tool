...
const positionWithinElement = (x: number, y: number, element: ElementType) => {
  ...
  switch (type) {
    ...
    case Tools.circle: {
      ...
      const distanceFromCenter = distance({ x: centerX, y: centerY }, { x, y });

      if (Math.abs(distanceFromCenter - radius) < 5) return "border"; // Near the edge
      if (distanceFromCenter < radius) return "inside"; // Inside the circle
      return null;
    }
    case Tools.pencil: {
